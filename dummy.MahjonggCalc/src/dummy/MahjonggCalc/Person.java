...
//	private Cursor mCursor;
    ...
    public static void startPickActivityForResult(Activity activity, int requestCode) {
        Intent intent = new Intent(Intent.ACTION_PICK, ContactsContract.Contacts.CONTENT_URI);
        activity.startActivityForResult(intent, requestCode);
    }

    public static Person fromActivityResult(Activity activity, Intent data) {
        Uri contactData = data.getData();
        int id = (int) ContentUris.parseId(contactData);
        Log.d(TAG, "id: " + id);

        return new Person(activity, id);
    }
